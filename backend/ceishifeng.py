...
# 2. 台风识别核心代码
def detect_typhoon_centers(time_step, data_quality=-9, merge_distance_km=150):
    """
    识别台风中心
    ...
    参数:
        ...
        merge_distance_km: 近距离合并半径（公里），将彼此过近的极大值合并为一个中心
    ...
    # 合并距离过近的候选中心，避免一个台风被拆成多个点
    def merge_close_centers(points, dx_m, dy_m, merge_km):
        if not points:
            return points
        used = [False] * len(points)
        merged = []
        for idx_a, a in enumerate(points):
            if used[idx_a]:
                continue
            group = [a]
            used[idx_a] = True
            for idx_b in range(idx_a + 1, len(points)):
                ...
                if dist_km <= merge_km:
                    ...
                    used[idx_b] = True
            # 取组内平均作为合并后的中心
            mean_i = int(round(sum(p[0] for p in group) / len(group)))
            mean_j = int(round(sum(p[1] for p in group) / len(group)))
            merged.append((mean_i, mean_j, int(time_step)))
        return merged

    centers = merge_close_centers(centers, dx, dy, merge_distance_km)
...
def track_typhoon_centers(initial_centers, start_time_step, end_time_step, max_search_radius=500, data_quality=-9, time_step_interval=1):
    """
    追踪台风中心在所有时间步中的位置
    ...
    参数:
        ...
        time_step_interval: 时间步间隔，每隔多少个时间步分析一次（默认1，即每个都分析）
    ...
    analyzed_steps = (total_steps + time_step_interval - 1) // time_step_interval
    print(f"时间步范围: {start_time_step} 到 {end_time_step} (共 {total_steps} 个时间步)")
    if time_step_interval > 1:
        print(f"时间步间隔: {time_step_interval} (实际分析 {analyzed_steps} 个时间步)")
    ...
    # 生成要分析的时间步列表（根据间隔）
    time_steps_to_analyze = list(range(start_time_step, end_time_step + 1, time_step_interval))
    if time_steps_to_analyze[0] != start_time_step:
        time_steps_to_analyze.insert(0, start_time_step)
    if time_steps_to_analyze[-1] != end_time_step:
        time_steps_to_analyze.append(end_time_step)
    time_steps_to_analyze = sorted(list(set(time_steps_to_analyze)))  # 去重并排序
    
    print(f"将分析的时间步: {time_steps_to_analyze[:10]}{'...' if len(time_steps_to_analyze) > 10 else ''} (共 {len(time_steps_to_analyze)} 个)")
    
    # 遍历后续时间步（按间隔）
    prev_analyzed_step = start_time_step
    for time_step in time_steps_to_analyze:
        if time_step == start_time_step:
            continue  # 跳过初始时间步，已经分析过了
        
        # 填充中间跳过的时间步（用None或插值）
        skipped_steps = time_step - prev_analyzed_step - 1
        if skipped_steps > 0:
            # 为所有追踪的台风填充跳过的时间步
            for typhoon_id in tracks.keys():
                tracks[typhoon_id].extend([None] * skipped_steps)
        ...
        try:
            ...
            if len(new_centers) == 0:
                ...
                current_centers = {typhoon_id: None for typhoon_id in tracks.keys()}
                prev_analyzed_step = time_step
                continue
            ...
            for idx, new_center in enumerate(new_centers):
                if idx not in matched_new_centers:
                    ...
                    current_length = len(tracks[list(tracks.keys())[0]]) if tracks else 0
                    tracks[new_typhoon_id] = [None] * current_length + [new_center]
    ...
    # 填充最后的时间步（如果end_time_step不在分析列表中）
    if prev_analyzed_step < end_time_step:
        remaining_steps = end_time_step - prev_analyzed_step
        for typhoon_id in tracks.keys():
            tracks[typhoon_id].extend([None] * remaining_steps)
    
    return tracks

def export_tracks_to_csv(tracks, start_time_step, end_time_step, output_filename="AAAA.csv"):
    """
    将台风追踪结果导出到CSV文件
    
    参数:
        tracks: 追踪结果字典，键为台风ID，值为位置列表
        start_time_step: 起始时间步
        end_time_step: 结束时间步
        output_filename: 输出文件名，默认为"AAAA.csv"
    ...
    for typhoon_id, trajectory in tracks.items():
        # 遍历轨迹中的每个位置
        for time_idx, pos in enumerate(trajectory):
            if pos is not None:
                i, j, t = pos  # i是行，j是列，t是时间步
                csv_data.append({
                    'time': t,
                    'x': j,  # 列索引作为x
                    'y': i,  # 行索引作为y
    ...
    # 按时间步排序
    csv_data.sort(key=lambda x: (x['time'], x['typhoon_name']))
    
    # 写入CSV文件
    try:
        with open(output_filename, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = ['time', 'x', 'y', 'typhoon_name']
        ...
        print(f"\n{'='*60}")
        print(f"追踪结果已导出到: {output_filename}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print(f"\n使用默认数据集URL: {field}")
        print("提示: 可以指定变量名，如: python ceishifeng.py salt")
        print("     或只加载风场: python ceishifeng.py uv")
        print("     或指定完整URL: python ceishifeng.py <URL>")
        print("     或指定时间步范围: python ceishifeng.py uv 0-100")
        print("     或指定时间步范围和间隔: python ceishifeng.py uv 0-100:5  (每隔5个时间步分析一次)")
    ...
    try:
        ...
        time_step_interval = 1  # 默认每个时间步都分析
        
        if len(sys.argv) > 2:
            arg2 = sys.argv[2].lower()
            if arg2 not in ["track", "追踪", "t"]:
                # 尝试解析为时间步范围，支持格式：0-100:5（从0到100，每隔5个时间步）
                if ":" in arg2:
                    # 包含间隔参数
                    try:
                        range_part, interval_part = arg2.split(":")
                        time_step_interval = int(interval_part)
                        if "-" in range_part:
                            start_time_step, end_time_step = map(int, range_part.split("-"))
                        elif range_part.isdigit():
                            end_time_step = int(range_part)
                        # 如果指定了时间步范围，自动启用追踪模式
        ...
        # 检查第三个参数是否也是间隔参数（向后兼容）
        if len(sys.argv) > 3:
            arg3 = sys.argv[3]
            if arg3.isdigit() and ":" not in sys.argv[2]:
                # 如果第二个参数没有包含间隔，第三个参数可能是间隔
                try:
                    time_step_interval = int(arg3)
                    if time_step_interval > 1:
                        print(f"\n使用时间步间隔: {time_step_interval} (每隔 {time_step_interval} 个时间步分析一次)")
                except:
                    pass
        ...
        if enable_tracking and end_time_step == 0:
            try:
                ...
                total_steps = end_time_step - start_time_step + 1
                analyzed_steps = (total_steps + time_step_interval - 1) // time_step_interval
                print(f"\n追踪模式: 将分析时间步 {start_time_step} 到 {end_time_step}")
                if time_step_interval > 1:
                    print(f"   时间步间隔: {time_step_interval} (共 {total_steps} 个时间步，实际分析 {analyzed_steps} 个)")
                else:
                    print(f"   (共 {total_steps} 个时间步)")
            except:
            ...
            total_steps = end_time_step - start_time_step + 1
            analyzed_steps = (total_steps + time_step_interval - 1) // time_step_interval
            print(f"\n追踪模式已启用: 将分析时间步 {start_time_step} 到 {end_time_step}")
            if time_step_interval > 1:
                print(f"   时间步间隔: {time_step_interval} (共 {total_steps} 个时间步，实际分析 {analyzed_steps} 个)")
            else:
                print(f"   (共 {total_steps} 个时间步)")
        ...
        if enable_tracking and len(initial_centers) > 0:
            ...
            tracks = track_typhoon_centers(
                ...
                time_step_interval=time_step_interval  # 时间步间隔
            )
            ...
            export_tracks_to_csv(tracks, start_time_step, end_time_step, csv_filename)
        else:
            # 只分析单个时间步，也导出到CSV
            if len(initial_centers) > 0:
                # 将单个时间步的结果转换为tracks格式
                single_tracks = {}
                for idx, center in enumerate(initial_centers):
                    typhoon_id = f"台风_{idx+1}"
                    single_tracks[typhoon_id] = [center]
                
                csv_filename = "AAAA.csv"
                export_tracks_to_csv(single_tracks, start_time_step, start_time_step, csv_filename)
