...
def process_face(face):
    ...
    # 只取前 30 天
    max_timestep = min(len(timesteps), DAYS_TO_TEST * HOURS_PER_DAY)

    results_face = []
    for idx in range(0, max_timestep, TIME_STEP_INTERVAL):
        t = timesteps[idx]
...
if __name__ == "__main__":
    ...
    # 保存
    with open("TC_time_space_cube_6faces_1month.json", "w") as f:
        json.dump(merged_list, f, indent=2)

    print("Saved → TC_time_space_cube_6faces_1month.json")
