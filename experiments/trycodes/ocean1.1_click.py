...
def build_grid(data_local, scale_xy=1.0):
    ...
    z = np.linspace(0, 1000, nz_grid)  # 保持深度比例
...
# ----------------------------
# 6️⃣ 鼠标点击两点生成截面（改用 slice_along_line 正确 API）
# ----------------------------
clicked_points = []
...
def click_callback(point, _):
    ...
    if len(clicked_points) == 2:
        ...
        # slice_along_line 正确使用 generate_triangles
        slice_data = grid.slice_along_line(line, generate_triangles=True)
