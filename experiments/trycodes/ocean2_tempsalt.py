...
# 自定义不透明度函数（让表面 & 下层都有层次感）
opacity_tf = [0.0, 0.02, 0.1, 0.25, 0.5, 0.8, 1.0]

vol = plotter.add_volume(
    grid,
    cmap="Blues",            # 蓝色 colormap
    opacity=opacity_tf,      # 不透明度曲线
    shade=True
)
...
def click_callback(point, _):
    global line_actor, point_actors

    # ⬤ 标记点
    sphere = pv.Sphere(radius=5, center=point)
    actor = plotter.add_mesh(sphere, color="yellow")
    ...
    if len(clicked_points) == 2:
        ...
        # ⬤ 添加连线
        line = pv.Line(p1, p2)
        line_actor = plotter.add_mesh(line, color="red", line_width=4)

        # 生成 100 个采样点沿线
        line_x = np.linspace(p1[0], p2[0], 200)
        line_y = np.linspace(p1[1], p2[1], 200)
        nx, ny, nz = data0.shape
        salt_along_line = np.zeros((nz, len(line_x)))

        # 映射回索引
        ix = np.clip(np.round((line_x / scale_xy - lon_start) * (ny - 1) /
                              (lon_end - lon_start)).astype(int), 0, ny - 1)
        iy = np.clip(np.round((line_y / scale_xy - lat_start) * (nx - 1) /
                              (lat_end - lat_start)).astype(int), 0, nx - 1)
