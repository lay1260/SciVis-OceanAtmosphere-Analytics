...
X = X.transpose(1, 0, 2)
Y = Y.transpose(1, 0, 2)
Z = -Z.transpose(1, 0, 2)  # 向下为正

# ----------------------------
# 5️⃣ PyVista 3D流线可视化（兼容 0.46.4）
# ----------------------------
...
# 种子点 PolyData
seed_points = pv.PolyData(grid.points[::50])  # 选取每50个点作为流线种子

# 使用 streamlines_from_source
streamlines = grid.streamlines_from_source(source=seed_points,
                                           vectors='velocity',
