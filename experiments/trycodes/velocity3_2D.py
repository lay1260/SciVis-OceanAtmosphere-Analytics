...
def onclick(event):
    ...
    if len(clicked_points) == 2:
        ...
        # ----------------------------
        # ----------------------------
        # 7️⃣ 绘制截面流线（箭头长度随速度大小变化）
        # ----------------------------
        X, Z = np.meshgrid(line_dist, z_grid)
        speed_magnitude = np.sqrt(speed_along ** 2 + speed_depth ** 2)

        # 对箭头长度进行归一化，控制箭头大小范围
        arrow_scale = 5  # 可调整体箭头大小
        arrow_lengths = (speed_magnitude / speed_magnitude.max()) * arrow_scale + 0.5  # 避免太小

        plt.figure(figsize=(10, 4))
        strm = plt.streamplot(X, Z, speed_along, speed_depth,
                              density=2.0,
        ...
        # 通过quiver绘制可随速度缩放的箭头
        # 注意：streamplot自带箭头，但大小固定，下面用quiver增强
        skip = (slice(None, None, 4), slice(None, None, 4))  # 每4个点绘制一次箭头，避免过密
        plt.quiver(X[skip], Z[skip],
                   speed_along[skip], speed_depth[skip],
                   speed_magnitude[skip],
                   cmap='viridis', scale=10, width=0.005)

        plt.gca().invert_yaxis()
        plt.colorbar(label='Velocity magnitude (m/s)')
        plt.xlabel('Distance along line (km)')
        plt.ylabel('Depth (m)')
        plt.title('Velocity streamlines along cross-section (arrow size ~ speed)')
        plt.show()
